...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        # Create a unique group name based on sender and receiver
    ...
    # Fetch previous messages from the database
        messages = await database_sync_to_async(list)(
            ...
            ).order_by('timestamp')  # Ensure to replace 'timestamp' with your actual timestamp field.
        )
        ...
        return await database_sync_to_async(lambda: [
            {
                ...
                'authuser': msg.sender.username  # Or another field that identifies the user
            }
            for msg in messages
        ])()
...
class SecondConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        connected = Connected(
            connected=await database_sync_to_async(User.objects.get)(id=self.sender),
        )
    ...
    async def remove_connected_user(self, user_id):
        await database_sync_to_async(
            lambda: Connected.objects.filter(connected_id=user_id).delete()
        )()
